...
uniform vec3 light0_direction; // need

void getNormal() {
		dNormalW = normalize(vNormalW); // need
}
...
uniform vec3 view_position;
vPositionW    = getWorldPosition(); = dPositionW
dPositionW = posW.xyz;
vec4 posW = dModelMatrix * vec4(vertex_position, 1.0);
dModelMatrix = uniform mat4 matrix_model;
attribute vec3 vertex_position;

## dNormalW 
	= normalize(vNormalW)
	vNormalW
		= normalize(dNormalMatrix * vertex_normal);
		dNormalMatrix
			= uniform mat3 matrix_normal;    == THREE uniform mat3 normalMatrix;
		vertex_normal 
			= attribute vec3 vertex_normal;  == THREE attribute vec3 normal;

#### dNormalW = normalize(matrix_normal * vertex_normal)

==> dNormalW = normalize ( normalMatrix * normal )


## vec3 dViewDirW;
	dViewDirW = normalize(view_position - vPositionW);
		view_position
			= uniform vec3 view_position;     == THREE uniform vec3 cameraPosition;
		vPositionW
			= getWorldPosition();
			= return dPositionW;
			= dPositionW = posW.xyz;
			= vec4 posW = dModelMatrix * vec4(vertex_position, 1.0);
				mat4 dModelMatrix;
					= dModelMatrix = getModelMatrix();
					= return matrix_model;
					= uniform mat4 matrix_model;      == THREE uniform mat4 modelMatrix;
				vertex_position
					= attribute vec3 vertex_position; == THREE attribute vec3 position;

===> dViewDirW = normalize ( cameraPosition -  modelMatrix * vec4(position, 1.0) )
